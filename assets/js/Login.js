...
function register() {
  ...
  if (mdp.length < 8) {
    createSidePopUp(
      "Merci de vous assurer que votre mot de passe contient au moins 8 caractères.",
      "error"
    );
  } else if (mdp != mdp1) {
    createSidePopUp(
      "Merci de vous assurer que les mots de passe sont égaux.",
      "error"
    );
  } else {
    ...
    xhr.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        if (this.responseText == 0) {
          ...
          createSidePopUp(
            "Il y a une erreur dans le remplissage des champs. Veuillez vérifier les informations que vous avez saisies et les corriger si nécessaire.",
            "error"
          );
  ...
  xhr.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (this.responseText == 1) {
        alert("Error");
        //error.innerHTML = "Login failed again and again";
      } else {
        var redirect = "index";
